...
for i in range(0,comp_datalist['stock_symbol'].count()):
    try:
        ...
        print("[Status]Processing TA Momentum Indicator for "+comp_datalist['stock_symbol'][i])
        ...
        df_save.to_csv(current_dir+"/"+base_dir+"/"+out_dir+'/'+comp_datalist['stock_symbol'][i]+"/"+out_dir+'_ta_momentum_indicator_'+comp_datalist['stock_symbol'][i]+'.csv',index=False)


    except:
        print("[Status]Error fail to processed TA Momentum Indicator for "+comp_datalist['stock_symbol'][i])